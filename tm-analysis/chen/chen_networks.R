...
# positive and negative correlations (4,10)
k=10
b_plot <- b_df %>%
...
# reducing all nosignificant to correlation of 0 to remove from plot
...
net3 %v% "Normalized Mean Abundance" = order(round(log2_df2$baseMean,0))
net3 %v% "LFC MS vs HC" = (round(abs(log2_df2$log2FoldChange),1))
net3 %v% "Abun2" = log2_df2$color
net3 %v% "Probability of Assignment" = round((b_plot2$beta),2)
...
ggsave(paste0("chen_network_topic_lfc",k,".png"),plot = last_plot(),
       device = 'png',
       height = 12,
       width = 14,
       dpi = 600)
...
# all positive correlation networks
# 6,23,5
...
log2_df2 <- log2_df2 %>% 
  mutate(lfc2 = (round(abs(log2_df2$log2FoldChange),1)))
...
a <- as.matrix(trunc(a,prec=2)) # cannot have negatives edges, will supply pos/neg later
...
ggsave(paste0("chen_network_topic_lfc_",k,".png"),plot = last_plot(),
       device = 'png',
       height = 12,
       width = 14,
       dpi = 600)
