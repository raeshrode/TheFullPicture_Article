...
# write.csv(tm_df,"Yadav_document_topic_mat.csv")
...
linda_rrms <- data.frame(tm_linda$output$GroupMS) %>%
...
(p2 <- ggplot(data = fdr_linda, aes(x = Topic, y = log2FoldChange, fill = Reject)) +
    geom_col(alpha = .8) +
    ggtitle("MS vs HC (Yadav)", subtitle =  paste0("p ≤ ",p," & q ≤ ", q)) +
    labs(y = "\nLog2 Fold-Change", x = "") +
    theme_bw() +
...
for (t in 1:length(sig_topics)) {
  ...
  (p3 <- ggplot(data = b_plot2, aes(x = beta, y = Genus, color = term)) +
      ...
      ggtitle(paste0("Community Type ",k, " (Yadav)"), subtitle = paste0("Log2FC ", lfc_k, " in ", r, " (p=",p," q=", q,")")) +
...
colnames(uni_plot_data[,2:54])
u_long <- uni_plot_data %>% 
  pivot_longer(., cols = 2:54)
...
# save for later
# write.csv(u_long_meta,"long_meta_data_yadav.csv")
...
for (t in 1:length(sig_topics)) {
  ...
  (
    p5 <-   topic_features2 %>% 
      ggplot(aes( y = value, x = Genus, color = Group, fill = Group)) +
...
topic_term_matrix <- b_df %>%  pivot_wider(names_from = topic,
                                           values_from = beta)
colnames(topic_term_matrix) <- paste0("yadav_",colnames(topic_term_matrix))
# write.csv(topic_term_matrix,"yadav_topic_term_matrix.csv", row.names = F)

# correlation for network #
ms_genus <- subset_samples(ps2_genus,Group=="MS")
...
# write.csv(cor_df_clean,"yadav_correlation.csv")
